...
def _compose(args):
    # Unpack the tuple inside the function
    method, state, cgpm_metadata, args = args
    builder = getattr(
...
def _evaluate(allargs):
    # Unpack the tuple inside the function
    method, state, args = allargs
...
class Engine(object):
    ...
    def transition_loom(self, N=None, S=None, kernels=None,
        ...
        from cgpm.src.crosscat import loomcat
    ...
    def to_metadata(self):
        metadata = dict()
        metadata['X'] = self.states[0].data_array()
