...
def convert_to_string(item):
    if isinstance(item, bytes):
        ...
        return {convert_to_string(k): convert_to_string(v) for k, v in item.items()}
    else:
        return item
def _crosscat_M_c(state):
    ...
    return {
        ...
        str('idx_to_name'):
            dict(zip(map(str, range(ncols)), column_names)),
        str('column_metadata'):
            column_metadata,
    }
...
def _crosscat_X_L(state, M_c, X_D):
    ...
    if state.Ci:
        from crosscat.src.utils.general_utils import get_scc_from_tuples
...
def _update_state(state, M_c, X_L, X_D):
    ...
    # pre process X_L to convert
    X_L = {convert_to_string(k): convert_to_string(v) for k, v in X_L.items()}
...
def transition(
    ...
    from crosscat.src.LocalEngine import LocalEngine
